...
public class CaseController {
	...
	@PostMapping("/apply")
	Result apply(@RequestBody CaseApplyWithCaseName can, HttpSession session) {
		User loginUser = (User) session.getAttribute("userinfo");
		String employeeId = loginUser.getUsername();
		can.setEmployeeId(employeeId);
		return caseService.apply(can);
	}
	...
	Result findOneCaseWithCurrEmp(@RequestBody CaseBase cb, HttpSession session) {
		...
		// 顧客情報
		CustomerCaseCount ccc = new CustomerCaseCount();
		ccc.setCaseCount(0);
		ccc.setCustomerId(c.getCustomerId());
		ArrayList<CustomerCaseCount> cl = (ArrayList<CustomerCaseCount>) customerService.findCustomer(ccc).getData();
		CustomerCaseCount findedCust = cl.get(0);
		Customer cust = new Customer();
		cust.setUsername(findedCust.getUsername());
		cust.setAddr1(findedCust.getAddr1());
		cwce.setCust(cust);
	...
	@GetMapping("/manager/cust/cases/{customerId}")
	Result getCasesByCustomerId(@PathVariable Integer customerId) {
		CaseBase cb = new CaseBase();
		cb.setCustomerId(customerId);
		return caseService.findCaseBase(cb);
	}
	
	@GetMapping("/manager/emp/cases/{employeeId}")
	Result getCaseByEmployeeId(@PathVariable String employeeId) {
		return caseService.findBindedCasesByEmployeeId(employeeId);
	}
	
	@PostMapping("/cust/finish")
	Result caseFinish(@RequestBody CaseBase cb) {
		return caseService.caseFinish(cb);
	}
	
	@GetMapping("/cust/finish/{caseId}")
	Result deleteCase(@PathVariable Integer caseId) {
